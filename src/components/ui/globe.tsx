  ...
  useEffect(() => {
    if (!globeRef.current && groupRef.current) {
      try {
        ...
        console.log("ThreeGlobe created:", globeRef.current);
        
        groupRef.current.add(globeRef.current);
        console.log("ThreeGlobe added to group");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (globeMaterial) {
        ...
        console.log("Globe material updated");
      }
    } catch (error) {
      console.warn("Could not update globe material:", error);
    }
  }, [
    isInitialized,
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (countries && countries.features && countries.features.length > 0) {
        console.log("Setting up hex polygons with", countries.features.length, "countries");
        globeRef.current
        ...
        console.log("Hex polygons configured");
      } else {
      ...
      if (data && data.length > 0) {
        console.log("Setting up arcs with", data.length, "connections");
        globeRef.current
          .arcsData(data)
        ...
        console.log("Arcs configured");
      }

      // Set up points
      if (filteredPoints && filteredPoints.length > 0) {
        console.log("Setting up points with", filteredPoints.length, "locations");
        globeRef.current
      ...
      console.log("Rings configured");

      console.log("Globe data built successfully");
    } catch (error) {
      console.error("Error building globe data:", error);
      setError("Error building globe data");
    }
  }, [
    isInitialized,
    data,
...
export function World(props: WorldProps) {
  ...
  console.log("World component rendering with config:", globeConfig);
